...
public class ReservationComplete extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		javaBean.setRoomId(request.getParameter("room"));
		javaBean.setDateFinish(dateEnd);
		javaBean.setDateStart(dateStart);
		javaBean.setConsumerEmail(request.getParameter("pEmail"));
		javaBean.setDescription(request.getParameter("Description"));
		
		RoomManager roomMg;
		try {
			ReservationManager resManager = new ReservationManager();

			javaBean.setId(resManager.getNextId());
			...
			response.sendRedirect("JSP/Reservar.jsp");
		}
